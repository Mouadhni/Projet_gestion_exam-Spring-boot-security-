...
const MultiSelectModule: React.FC<DropdownProps> = ({ id, enseignants, numberOfSelections = Infinity, onSelectionChange }) => {
  ...
  useEffect(() => {
    const newOptions = enseignants.map((enseignant) => ({
      value: enseignant.id,
      text: `${enseignant.id} - ${enseignant.nom}`,
      selected: false,
    }));
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    const selectedIds = updatedSelected.map((selectedIndex) => options[selectedIndex].value);
