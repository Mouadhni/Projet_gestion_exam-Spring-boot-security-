...
const FormExamenUpdate = () => {
  ...
  useEffect(() => {
    ...
    if (examenToUpdateString) {
      ...
      setSemestre(elemntToUpdate.element.semestre || '');
      setSession(elemntToUpdate.element.sesion || '');
      setType(elemntToUpdate.element.type || '');
      setDate(elemntToUpdate.element.date || '');
      setHeure(elemntToUpdate.element.heur || '');
      setDurerReel(elemntToUpdate.element.dure_reel || '');
      setDurerPrevue(elemntToUpdate.element.dure_prevu || '');
      setAnneeUniversitaire(elemntToUpdate.element.anneUniversitaire || '');
      setSelectedEnseignants(elemntToUpdate.element.coordonnateur_id ? [elemntToUpdate.element.coordonnateur_id.toString()] : []);
      setSelectedModules(elemntToUpdate.element.module_id ? [elemntToUpdate.element.module_id.toString()] : []);
      setEpreuveBase64(elemntToUpdate.element.epreuve ? elemntToUpdate.element.epreuve : null);
    }
  }, []);
  ...
  console.log(" testttt:  "+nom+" "+coordonnateur_id+" "+ module_id+" "+ semestre+" "+sesion+" "+ type+" "+ date+" "+ heur+" "+ dure_reel+" "+ dure_prevu+" "+anneUniversitaire+"---------"+epreuve )
    axios.put(
      `http://localhost:8080/login/admin/UpdateExamen/${id}`,
...
export default FormExamenUpdate;
