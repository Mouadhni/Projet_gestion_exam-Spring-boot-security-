...
const MultiSelect: React.FC<DropdownProps> = ({ id, enseignants, numberOfSelections = Infinity, onSelectionChange }) => {
  ...
  useEffect(() => {
    const newOptions = enseignants.map((enseignant, index) => ({
      value: enseignant.id,
      text: `${enseignant.id} - ${enseignant.username}`,
      selected: false,
    }));
        ...
        <select className="hidden" id={id}>
          {enseignants.map((enseignant) => (
            <option key={enseignant.id} value={enseignant.id}>
              {`${enseignant.id} - ${enseignant.username}`}
            </option>
          ))}
        </select>
