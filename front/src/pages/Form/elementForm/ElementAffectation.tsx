...
const ElementAffectation = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const enseignant_id = selectedEnseignants;
    const module_id = selectedModules.length > 0 ? selectedModules[0] : '';
  
    axios.post(
      'http://localhost:8080/login/admin/affecterElement',
      { enseignant_ids:enseignant_id,element_id:module_id},
      {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
        },
...
export default ElementAffectation;
